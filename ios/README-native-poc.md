...
- Le webview charge l'app hébergée. Un script injecté (`AppDelegate.swift`)
  ...
  - `POST /api/reports/day/print` (X/Z, via le rendu ticket `/pdf?ticket=1`)
  - `POST /api/cash-sessions/open-drawer` -> impulsion tiroir ESC/POS
    (uniquement si CloudPRNT n'a pas déjà envoyé l'impulsion).

  L'impression est réessayée jusqu'à 3 fois (imprimante en veille / réseau
  instable) avant de renvoyer une erreur à la caisse.
...
- **Factures** (`/api/invoices`) : format A4 (courrier), non pertinent en
  thermique. Elles restent en PDF / email.
